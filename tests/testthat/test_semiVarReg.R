...
test_that("Checking linear mean & variance model", {
  ...
  #expect_equal_to_reference(linse, "se1.rds") ##cannot check bootreps as too variable
...
test_that("Checking con mean model", {
  ...
  plotVarReg(conmeanmodel, ci=FALSE, maxit=100)
  plotVarReg(x=conmeanmodel, ci=TRUE, ci.type = "boot", bootreps = 2, maxit=100)
})
test_that("Checking zero mean model", {
  ...
  plotVarReg(x=zeromeanmodel, ci=TRUE, ci.type = "boot", bootreps = 2, maxit=100)
})
