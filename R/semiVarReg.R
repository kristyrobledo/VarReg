...
#'
#' \code{semiVarReg} performs a semiparametric mean and variance regression model. Currently, this is only designed for a single covariate that is fit in the mean and variance models.
...
#' @param meanmodel Specify the mean model to be fit: "zero" =zero mean model, "constant" = constant mean, "linear" = linear term with x covariate, "semi" = semiparametric spline (specify with \code{knots.m}).
#' @param varmodel Specify the variance model to be fit: "constant" = constant variance model, "linear" = linear term with x covariate, "semi" = semiparametric spline (specify with \code{knots.v})
...
#' @return \code{semiVarReg} returns an object of class "\code{VarReg}" which inherits some components from the class "\code{glm}".
#'
#' Includes converge, reldiff, information criterion and mean and variance estimates.
#'@examples
#'data(lidar)
...
semiVarReg<-function(y,x, meanmodel=c("zero", "constant", "linear", "semi"), varmodel=c("constant", "linear", "semi"), knots.m=2, knots.v=2, degree=2, mono.var=c("none", "inc", "dec"), control=list(...), ...){
