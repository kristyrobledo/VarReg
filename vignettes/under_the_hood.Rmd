...
```{tikz,ecmemean, echo=FALSE, dev.args=list(pointsize=100), fig.cap="The ECME algorithm for the estimation of the mean and variance",fig.align="center", fig.dim=c(5,5), fig.ext='png'}
...
\tikzstyle{terminator} = [draw, 
    ...
    \node [terminator] (init) {Initialise with $\hat{\boldsymbol{\beta}}^{old}$ and $\hat{\boldsymbol{\alpha}}^{old}$};  
    \node [block, below of=init] (latent)  {calculate $\hat{Y}_i^2, \hat{Z}_{i1}^2,..., \hat{Z}_{iQ}^2$ given $\hat{\boldsymbol{\beta}}^{old}$ and $\hat{\boldsymbol{\alpha}}^{old}$};  
    \node [block, below of=latent] (cala) {calculate updated estimates $\hat{\boldsymbol{\alpha}}^{new}$};  
    \node [block, below of=cala] (wLS) {fit weighted LS given updated $w_i=\frac{1}{\sigma_i^2(\hat{\boldsymbol{\alpha}}^{new})}$};  
    \node [block, below of=wLS] (calb) {calculate $\hat{\boldsymbol{\beta}}^{new}$ from the weighted LS}; 
     \node [decision, below of=calb] (conv) {check convergence}; 
\node [hex, below of=conv] (fin) {finish}; 
\node[block2, left of=wLS, node distance=8cm](update){set $\hat{\boldsymbol{\beta}}^{old}=\hat{\boldsymbol{\beta}}^{new}$ and $\hat{\boldsymbol{\alpha}}^{old}=\hat{\boldsymbol{\alpha}}^{new}$};
  
     \path [line] (init)  -- (latent);  
    \path [line] (latent)  -- (cala);  
    \path [line] (cala) -- (wLS);  
    \path [line] (wLS) -- (calb);  
    \path [line] (calb) -- (conv);  
    \path [line] (conv.south)-- node [right] {Yes} (fin);
    \path [line] (conv.west)-| (update.south)
            node[near start,above] {No} (fin);
    \path [line] (update.north) |- (latent.west);          
    \end{tikzpicture}
...
```{tikz,ecmecensored, echo=FALSE, dev.args=list(pointsize=100), fig.cap="The ECME algorithm for the estimation of the mean and variance with censored outcome data",fig.align="center", fig.dim=c(5,5), fig.ext='png'}
\usetikzlibrary{shapes.geometric,arrows}
...
\tikzstyle{terminator} = [draw, 
    ...
    \node [terminator] (init) {Initialise with $\hat{\boldsymbol{\beta}}^{old}$ and $\hat{\boldsymbol{\alpha}}^{old}$};  
    \node [block, below of=init] (latent)  {calculate $\hat{Y}_i^2, \hat{Z}_{i1}^2,..., \hat{Z}_{iQ}^2$ given $\hat{\boldsymbol{\beta}}^{old}$ and $\hat{\boldsymbol{\alpha}}^{old}$ using (2.8) and (2.9)};  
    \node [block, below of=latent] (cala) {calculate updated estimates $\hat{\boldsymbol{\alpha}}^{new}$ using (2.10)};  
    \node [block, below of=cala] (wLS) {fit censored LS given $\hat{\boldsymbol{\alpha}}^{new}$ and updated standardized data (2.11)};  
    \node [block, below of=wLS] (calb) {use censored LS fit to obtain $\hat{\boldsymbol{\beta}}^{new}$}; 
     \node [decision, below of=calb] (conv) {check convergence}; 
\node [hex, below of=conv] (fin) {finish}; 
\node[block2, left of=wLS, node distance=8cm](update){set $\hat{\boldsymbol{\beta}}^{old}=\hat{\boldsymbol{\beta}}^{new}$ and $\hat{\boldsymbol{\alpha}}^{old}=\hat{\boldsymbol{\alpha}}^{new}$};
...
```{tikz,cyclicascent, echo=FALSE, dev.args=list(pointsize=100), fig.cap="The cyclic coordinate ascent algorithm for the estimation of the location, scale and shape",fig.align="center", fig.dim=c(5,5), fig.ext='png'}
\usetikzlibrary{shapes.geometric,arrows}
...
\tikzstyle{terminator} = [draw, 
    ...
    \node [terminator] (init) {initialize with $\hat{\boldsymbol{\xi}}^{old}$, $\hat{\boldsymbol{\omega}}^{old}$, $\hat{\boldsymbol{\nu}}^{old}$};  
    \node [block, below of=init] (latent)  {calculate $V_i$, $|r_i|$ and $u_{ik} \times |r_i|$ using fixed $\hat{\boldsymbol{\xi}}^{old}$ and $\hat{\boldsymbol{\omega}}^{old}$};  
    \node [block, below of=latent] (cala) {fit probit regression to obtain $\hat{\boldsymbol{\nu}}^{new}$};  
    \node [block, below of=cala] (wLS) {calculate $D_i$, $z_{ip}$, $x_{iq}$ using fixed $\hat{\boldsymbol{\nu}}^{new}$};  
    \node [block, below of=wLS] (calb) {fit censored regression model to obtain $\hat{\boldsymbol{\xi}}^{new}$ and  $\hat{\boldsymbol{\omega}}^{new}$ as per Figure 2.1}; 
     \node [decision, below of=calb] (conv) {check convergence}; 
\node [hex, below of=conv] (fin) {finish}; 
\node[block2, left of=wLS, node distance=8cm](update){set $\hat{\boldsymbol{\xi}}^{old}=\hat{\boldsymbol{\xi}}^{new}$, $\hat{\boldsymbol{\omega}}^{old}=\hat{\boldsymbol{\omega}}^{new}$, $\hat{\boldsymbol{\nu}}^{old}=\hat{\boldsymbol{\nu}}^{new}$};
